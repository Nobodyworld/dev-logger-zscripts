...
- **`sample_project/`** – Demonstrates how a multi-language repository behaves
...
- **Adding new stacks:** Extend `_PRESETS` in `zscripts/presets.py`. The CLI and
  ...
  overridden via `zscripts.config.json`.
- **Customising ignore rules:** Update `zscripts.config.json` to amend `skip`
