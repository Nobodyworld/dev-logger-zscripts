...
- **Observability Hub** (`zscripts/observability/`) provides structured logging,
  ...
  server now exposes its own counters and histograms (`zscripts_health_http_requests_total`,
  `zscripts_health_http_request_duration_seconds`, and
  `zscripts_health_http_requests_inflight`) so operators can see which probe is
  receiving traffic and how long responses take. The CLI activates telemetry
  when `telemetry_enabled` is true or `--enable-telemetry` is passed and records
  dedicated metrics (`zscripts_cli_invocations_total`,
  `zscripts_cli_duration_seconds`, `zscripts_operations_total`,
  `zscripts_operation_duration_seconds`, `zscripts_extensions_active`).
- **Extensions** live under `zscripts/extensions`. `ExtensionContext` exposes
  ...
  the service is ready with full observability. During load a manifest registry
  captures metadata (`name`, `module`, `capabilities`, `version`, `config_keys`)
  for every extension; the CLI surfaces the manifest via
  `python cli.py extensions --output-format json`, and templates generated by
  `scripts/scaffold_extension.py` populate the manifest automatically.
