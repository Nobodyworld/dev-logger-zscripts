...
Documentation describing architecture and automation workflows is stored at the repository root (`README.md`, `ARCHITECTURE_OVERVIEW.md`, `AUTOMATION.md`, `EXTENSION_GUIDE.md`). Release metadata is tracked in `CHANGELOG.md` and `RELEASE_NOTES.md`. Coverage artifacts are produced into `reports/` by `make coverage` or `scripts/dev_start.py`.
...
3. Documentation:
   - Update `README.md` and `ARCHITECTURE_OVERVIEW.md` sections describing telemetry to mention CLI-level metrics and correlation IDs.
   - Refresh `AUTOMATION.md` with instructions for scraping CLI metrics and verifying telemetry shutdown.
   - Extend `EXTENSION_GUIDE.md` with advice for extension authors about using the shared metrics registry.
...
4. Release notes & coverage:
   ...
   - Update `RELEASE_NOTES.md` with a concise narrative of these changes.
   - Ensure `reports/coverage.json` is generated by running the coverage command after tests to embed in version control as evidence.
...
- Execute `make coverage` to refresh `reports/coverage.json` and `coverage report` output.
