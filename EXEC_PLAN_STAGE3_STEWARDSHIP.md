...
- Observation: `scripts/bootstrap.py` and `scripts/agent_guard.py` provide deterministic bootstrapping and guardrail execution for agents.
  Evidence: New scripts added with doc updates in `CONTRIBUTING.md` and `AUTOMATION.md`.
...
- Coverage now sits at 90% with infrastructure, logging, and configuration modules
  ...
  artifact (`reports/quality_gate.json`) confirms lint, type, and coverage
...
5. Update `CHANGELOG.md` and `RELEASE_NOTES.md` with Stage 3 highlights, ensuring release instructions align with new automation.

## Validation and Acceptance

Acceptance requires:
- `make check` passes, invoking lint, type, test, security, and coverage checks.
- `pytest --cov=zscripts --cov-report=term-missing` reports ≥85% coverage for critical modules; document results in `reports/coverage_report.md`.
...
- Invoking `python cli.py extensions scaffold sample_future` creates a template extension consistent with `EXTENSION_GUIDE.md` and passes contract tests.
- Documentation (`ARCHITECTURE_OVERVIEW.md`, `EXTENSION_GUIDE.md`, `docs/operations.md`, `docs/future_roadmap.md`, `AUTOMATION.md`, `CONTRIBUTING.md`) reflects updated workflows and references new tooling.
- `scripts/update_dependencies.py --check` demonstrates dependency status and surfaces actionable output.
- Release artifacts (`CHANGELOG.md`, `RELEASE_NOTES.md`) summarize Stage 3 improvements and any migration notes.
