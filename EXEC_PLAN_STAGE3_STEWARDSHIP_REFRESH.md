...
3. **Developer Tooling & Docs**: Implement `scripts/ops_status.py` to fetch health endpoints, persist optional JSON summaries, and exit non-zero on degraded status. Document usage across `AUTOMATION.md`, `docs/operations.md`, `docs/operations/INCIDENT_RESPONSE.md`, `EXTENSION_GUIDE.md`, `ARCHITECTURE_OVERVIEW.md`, and `README.md` as needed. Capture future-proofing notes in `docs/future_roadmap.md` and update `CONTRIBUTING.md` with the telemetry/tooling workflow. Refresh `CHANGELOG.md` and `RELEASE_NOTES.md`.
...
- `python scripts/dev_start.py` succeeds, generating updated `reports/quality_gate.json` and `reports/coverage.json` showing ≥85% coverage.
