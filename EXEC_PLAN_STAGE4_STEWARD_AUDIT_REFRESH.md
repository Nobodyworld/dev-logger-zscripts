...
1. **Metrics Refresh**
   ...
   - [x] Regenerate `reports/metrics.json` (new artifact) for reproducible audits.
...
3. **Documentation Alignment**
   - [x] Update `README.md`, `AUTOMATION.md`, and `AUTOMATION_ROLES.md` to reference the
...
4. **Automation Artifacts**
   - [x] Ensure `reports/` captures the latest metrics including coverage snapshot.
   - [x] Provide `AUTOMATION_ROLES.md` updates and, if necessary, additional
     documentation describing agent responsibilities.

## Validation
- Run `ruff`, `mypy`, and `pytest` to confirm code health after modifications.
- Execute `python scripts/collect_quality_metrics.py --output reports/metrics.json` to
