...
`export_cli_metadata()` so orchestration frameworks can render prompts or
JSON schemas for the `collect`, `consolidate`, and `tree` commands. Each command
definition lists parameters, default values, and example invocations, while the
payload also includes the preset catalogue exported from `zscripts.presets`. Use
...
The resulting dictionary contains `commands` and `presets` keys ready for
serialization (for example, when publishing a MCP manifest).
