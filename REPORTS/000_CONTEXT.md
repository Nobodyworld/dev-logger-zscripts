...
- **Sample stacks:** `sample_project/` includes Python, JavaScript/TypeScript, shell, and YAML files for aggregation demonstrations; no runtime frameworks bundled beyond standard library modules.
...
- **Configuration:** Default runtime configuration stored in `zscripts.config.json`, consumed by `zscripts.config` helpers and CLI flags.
...
- `sample_project/` – Demonstration codebase mixing backend/front-end/infrastructure files for collection tests.
- `Makefile`, `noxfile.py`, `requirements.txt`, and `pyproject.toml` – Tooling definitions.
- Documentation and planning artefacts: multiple `EXEC_PLAN_*.md` files, `PROJECT_STATUS_REPORT.md`, `REPORT.md`, and logs inside `zscripts_logs/`.
