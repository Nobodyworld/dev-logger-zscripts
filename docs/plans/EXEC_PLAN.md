...
4. **Documentation refresh** - Rewrite `README.md` to highlight installation, usage examples, configuration schema, and development workflow. Update `docs/plans/PROJECT_STATUS_REPORT.md` to summarize status, backlog, and quality signals. Include CLI help snippet reflecting new options.
...
5. Add packaging/quality files: `pyproject.toml`, `ruff.toml`, `noxfile.py`, `.gitignore` updates if needed. Introduce `tests/` with pytest modules covering config loading, ignore logic, log creation, and CLI invocation against `examples/sample_project`.
6. Revise `README.md` and `docs/plans/PROJECT_STATUS_REPORT.md` with professional tone, usage instructions, tooling badges/information.
...
- Running `python -m zscripts collect --types python --project-root examples/sample_project` produces logs without stack traces, and `--dry-run` logs planned outputs without writing files.
