...
- Documentation (`README.md`, possibly `docs/plans/PROJECT_STATUS_REPORT.md`) lists CLI capabilities and should mention auto-detected project roots plus stricter config validation once implemented.
...
5. Amend `README.md` (and `docs/plans/PROJECT_STATUS_REPORT.md` if necessary) to describe the new CLI behaviour. Mention reporter-based consistent output only if user-facing (e.g., new warning message).
6. Run `ruff format .`, `ruff check .`, `mypy .`, `bandit -q -r zscripts examples/sample_project`, and `pytest` (or simply `make check`). Capture outputs for final report.
