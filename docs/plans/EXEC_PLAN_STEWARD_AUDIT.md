...
- [x] (2025-10-26T02:35Z) Authored `scripts/collect_quality_metrics.py` and captured coverage, complexity, dependency, and build outputs into `artifacts/quality/metrics.json`.
...
The repository root contains the CLI entry point at `zscripts/cli.py`, configuration helpers under `zscripts/configuration.py`, telemetry coordination inside `zscripts/observability/`, and tests in `tests/`. Documentation is spread across the root Markdown files (README, CHANGELOG, RELEASE_NOTES) and supporting reports within `docs/reports/narratives/REPORTS/` and `docs/`. Stage 3 previously introduced detailed telemetry instrumentation in `zscripts/cli.py`; our audit must ensure that instrumentation is correct yet simple, that metrics align with documentation, and that new stewardship artifacts live beside existing reports (e.g., `REPORT.md`).
...
2. Generate holistic metrics by executing `scripts/collect_quality_metrics.py --output artifacts/quality/metrics.json`; if the script does not exist, implement the AST-based complexity/dependency walker using stdlib modules.
...
2. Execute `python scripts/collect_quality_metrics.py --output artifacts/quality/metrics.json` to generate coverage, complexity, dependency, and build statistics.
...
7. Run `coverage report` (plus optional `ruff check`/`mypy`) and archive artefacts (`artifacts/coverage/coverage.json`, `artifacts/quality/metrics.json`).
...
- `python scripts/collect_quality_metrics.py --output artifacts/quality/metrics.json`: outputs captured in `artifacts/quality/metrics.json`.
