...
| Sample fixture | `examples/sample_project` polyglot monorepo used for integration tests & documentation screenshots. | `/examples/sample_project`. |
...
4. **Outputs**: Results are written under `examples/artifacts/zscripts_logs/` (default) or a caller-defined directory. Dry-run and verbose flags
...
├─ examples/sample_project/            # integration fixture (polyglot)
...
|---------|----------|-----------|--------|
| `zscripts collect` | Capture project artefacts grouped by stack preset. | `--target`, `--dry-run`, `--verbose`. | Files under `examples/artifacts/zscripts_logs/`. |
...
```
Tests exercise CLI/config/utils and rely on `examples/sample_project` fixtures.
...
- `configs/zscripts.config.json` appears unused (legacy artifact) — confirm via rg/tests before removal.
...
- Historical planning docs under `EXEC_PLAN_*` and `docs/reports/narratives/REPORTS/` duplicate the modern report/plan; candidates for archival.
...
## Evidence & Next Steps
- `docs/plans/STATUS.md` will capture progress per PR with risk/rollback notes.
