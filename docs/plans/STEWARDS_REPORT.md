...
| Metric | Value | Notes |
| --- | --- | --- |
| Test coverage | 88.46 % statements across 1,623 lines | Generated via `python scripts/dev_start.py`; see `artifacts/coverage/coverage.json`. |
...
All metrics are persisted to `artifacts/quality/metrics.json` so humans and agents can audit changes
...
- `artifacts/quality/metrics.json` accompanies `artifacts/quality/quality_gate.json` in CI artifacts, giving
  operators immediate visibility into coverage, complexity, dependency, build, and latency
  trends.
- `docs/automation/AUTOMATION.md` and `docs/automation/AUTOMATION_ROLES.md` direct agents to the new HTTP entrypoint tag on
