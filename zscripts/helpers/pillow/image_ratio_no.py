...
def process_folder(source_dir: Path, destination_root: Path) -> None:
    """Resize PNG files into predefined dimensions, optionally adding vertical margins."""
    ...
    for png_path in sorted(source_dir.glob("*.png")):
        ...
        for label, (dimensions, margin_inches) in SIZE_CONFIG.items():
            ...
            try:
                resized.save(output_file, format="JPEG", quality=92, dpi=DPI)
            except OSError as exc:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Resize PNGs into multiple aspect ratios.")
    parser.add_argument("input_dir", type=Path, help="Directory of source PNG files.")
    parser.add_argument(
        "output_dir",
        type=Path,
        help="Destination directory to receive per-image subfolders.",
    )
